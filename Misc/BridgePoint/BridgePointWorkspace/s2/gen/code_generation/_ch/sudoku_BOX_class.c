 ...
 */
static Escher_SetElement_s sudoku_BOX_container[ sudoku_BOX_MAX_EXTENT_SIZE ];
static sudoku_BOX sudoku_BOX_instances[ sudoku_BOX_MAX_EXTENT_SIZE ];
Escher_Extent_t pG_sudoku_BOX_extent = {
  {0}, {0}, &sudoku_BOX_container[ 0 ],
  (Escher_iHandle_t) &sudoku_BOX_instances,
  sizeof( sudoku_BOX ), sudoku_BOX_STATE_1, sudoku_BOX_MAX_EXTENT_SIZE
  };
 ...
 */
i_t
sudoku_BOX_op_prune( sudoku_BOX * self )
{
...
while ( 0 != ( CELL_R4 = (sudoku_CELL *) Escher_IteratorNext( &iCELL_R4 ) ) ) {
sudoku_DIGIT * selected = CELL_R4->DIGIT_R9;
...
while ( 0 != ( ELIGIBLE_R8 = (sudoku_ELIGIBLE *) Escher_IteratorNext( &iELIGIBLE_R8 ) ) ) {
  ...
  sudoku_ELIGIBLE * sudoku_ELIGIBLEiter14;
  Escher_IteratorReset( &iter14, eligibles );
  while ( (sudoku_ELIGIBLEiter14 = (sudoku_ELIGIBLE *)Escher_IteratorNext( &iter14 )) != 0 ) {
    eligible = sudoku_ELIGIBLEiter14; {
    sudoku_DIGIT * answerdigit;
    /* FOR EACH answerdigit IN answerdigits */
    { Escher_Iterator_s iter16;
  sudoku_DIGIT * sudoku_DIGITiter16;
  Escher_IteratorReset( &iter16, answerdigits );
  while ( (sudoku_DIGITiter16 = (sudoku_DIGIT *)Escher_IteratorNext( &iter16 )) != 0 ) {
    answerdigit = sudoku_DIGITiter16; {
...
Escher_Iterator_s iCELL_R4;
Escher_IteratorReset( &iCELL_R4, &self->CELL_R4 );
while ( 0 != ( selected = (sudoku_CELL *) Escher_IteratorNext( &iCELL_R4 ) ) ) {
  ...
  sudoku_CELL * sudoku_CELLiter15;
  Escher_IteratorReset( &iter15, opencells );
  while ( (sudoku_CELLiter15 = (sudoku_CELL *)Escher_IteratorNext( &iter15 )) != 0 ) {
    opencell = sudoku_CELLiter15; {
    i_t c;
 ...
 */
i_t
sudoku_BOX_op_eliminate( sudoku_BOX * self )
{
...
while ( 0 != ( ELIGIBLE_R8 = (sudoku_ELIGIBLE *) Escher_IteratorNext( &iELIGIBLE_R8 ) ) ) {
  ...
  sudoku_ELIGIBLE * sudoku_ELIGIBLEiter17;
  Escher_IteratorReset( &iter17, eligibles );
  while ( (sudoku_ELIGIBLEiter17 = (sudoku_ELIGIBLE *)Escher_IteratorNext( &iter17 )) != 0 ) {
    eligible = sudoku_ELIGIBLEiter17; {
 ...
 */
void
sudoku_BOX_R1_Link( sudoku_SEQUENCE * supertype, sudoku_BOX * subtype )
{
  if ( (supertype == 0) || (subtype == 0) )
  {
    ROX_EMPTY_HANDLE_TRACE( "BOX", "sudoku_BOX_R1_Link" )
    return;
  }
  ...
  supertype->R1_object_id = sudoku_BOX_CLASS_NUMBER;
}
 ...
 * State 1:  [solving]
 */
static void sudoku_BOX_act1( sudoku_BOX *, const Escher_xtUMLEvent_t * const );
static void
sudoku_BOX_act1( sudoku_BOX * self, const Escher_xtUMLEvent_t * const event )
{
  /* IF ( ( 100 == self.prune() ) ) */
  if ( ( 100 == sudoku_BOX_op_prune(self) ) ) {
    /* GENERATE BOX2:solved() TO self */
    {
  Escher_xtUMLEvent_t * e = Escher_NewxtUMLEvent( self, &sudoku_BOXevent2c );
  Escher_SendSelfEvent( e );
  }
  }
  else if ( ( 100 == sudoku_BOX_op_eliminate(self) ) ) {
    /* GENERATE BOX2:solved() TO self */
    {
  Escher_xtUMLEvent_t * e = Escher_NewxtUMLEvent( self, &sudoku_BOXevent2c );
  Escher_SendSelfEvent( e );
    ...
    if ( ( sequence->requests >= 1 ) ) {
      ...
      {
  Escher_xtUMLEvent_t * e = Escher_NewxtUMLEvent( b, &sudoku_BOXevent1c );
  Escher_SendEvent( e );
  }
    }
    else {
 ...
 * State 2:  [solved]
 */
static void sudoku_BOX_act2( sudoku_BOX *, const Escher_xtUMLEvent_t * const );
static void
sudoku_BOX_act2( sudoku_BOX * self, const Escher_xtUMLEvent_t * const event )
{
  sudoku_SEQUENCE * sequence; /* sequence (SEQUENCE) */
...
const Escher_xtUMLEventConstant_t sudoku_BOXevent1c = {
  sudoku_DOMAIN_ID, sudoku_BOX_CLASS_NUMBER, SUDOKU_BOXEVENT1NUM,
  ESCHER_IS_INSTANCE_EVENT };

const Escher_xtUMLEventConstant_t sudoku_BOXevent2c = {
  sudoku_DOMAIN_ID, sudoku_BOX_CLASS_NUMBER, SUDOKU_BOXEVENT2NUM,
  ESCHER_IS_INSTANCE_EVENT };
 ...
 */
static const Escher_SEMcell_t sudoku_BOX_StateEventMatrix[ 2 + 1 ][ 2 ] = {
  /* row 0:  uninitialized state (for creation events) */
  { EVENT_CANT_HAPPEN, EVENT_CANT_HAPPEN },
  /* row 1:  sudoku_BOX_STATE_1 (solving) */
  { sudoku_BOX_STATE_1, sudoku_BOX_STATE_2 },
  /* row 2:  sudoku_BOX_STATE_2 (solved) */
   ...
   */
  static const StateAction_t sudoku_BOX_acts[ 3 ] = {
    (StateAction_t) 0,
    (StateAction_t) sudoku_BOX_act1,  /* solving */
    (StateAction_t) sudoku_BOX_act2  /* solved */
  };
 ...
 */
void
sudoku_BOX_Dispatch( Escher_xtUMLEvent_t * event )
{
  ...
  if ( 0 != instance ) {
    ...
    if ( current_state > 2 ) {
      ...
      next_state = sudoku_BOX_StateEventMatrix[ current_state ][ event_number ];
      if ( next_state <= 2 ) {
        STATE_TXN_START_TRACE( "BOX", current_state, state_name_strings[ current_state ] )
        /* Execute the state action and update the current state.  */
        ( *sudoku_BOX_acts[ next_state ] )( instance, event );
